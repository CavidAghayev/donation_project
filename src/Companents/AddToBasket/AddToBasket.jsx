...
dispatch(decrement(productId))
  ...
  const total = cart.reduce((acc,item) =>
   acc + products.find((a) => a.id === item.id).price * item.count,0
   )
   console.log(total)
  return (
    <section>
...
cart.map((t) => {
    ...
    console.log(product)
    return <div className="add-to-cart-content" key={product.id}>
      <div className="add-to-cart-image">
      <img src={product.image} alt="" />
      </div>
      <div className="add-to-cart-items">
        <div className="add-to-cart-buttons">
        <button onClick={()=>decreaseItem(t.id)} className='sell'>-</button>
